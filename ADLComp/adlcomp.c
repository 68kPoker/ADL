...
void init()
{
    ...
    mktemp( tmp_c );
#endif
    S_VAL_F = open( tmp_s, WB, _S_IWRITE );
    CODE_F = open( tmp_c, WB, _S_IWRITE );
...
void write_code()
{
    ...
    temp = currcode();
    hdr.codeindex.numobjs = (int16) (temp / 512L) + 1;
    temp = numchar();
    hdr.strindex.numobjs = (int16) (temp / 512L) + 1;
    ...
    /* Write the file */
    if( (outf = open( outname, WB, _S_IWRITE )) < 0 )
